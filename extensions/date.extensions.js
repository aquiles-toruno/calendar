...
Date.prototype.isWeekend = function () {
    ...
    for (var key in daysOfWeek) {
        if (daysOfWeek.hasOwnProperty(key)) {
            var element = daysOfWeek[key];
            if (element.index === dayOfWeek) return element.isWeekend;
        }
    }
...
Date.prototype.getNumberOfWeeks = function () {
    var weekStartOnSunday = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    ...
    if (!weekStartOnSunday) used = used + 6;
